...
GLInstance::GLInstance(std::shared_ptr<SoftmaxSettings> aSettings)
{
	...
	window = glfwCreateWindow(aSettings->windowWidth, aSettings->windowHeight, "Cuda Softmax Regression", NULL, NULL);
	glfwSetWindowPos(window, 1920 - 2*aSettings->windowWidth - 2, 0);
	...
	colorIndexCount = aSettings->windowWidth * aSettings->windowHeight * 4;
	...
	for (int i = 0; i < colorIndexCount; i++){
		if(i%4==0){ //top left of a quad
			quadPoints[i].x = 2.0*(float)idxCol/aSettings->windowWidth - 1.0f + halfWidth;
			quadPoints[i].y = 1.0f - 2.0*(float)idxRow/aSettings->windowHeight - halfHeight;
			quadPoints[i].z = 0.0f;
		}
		if(i%4==1){ //bottom left of a quad
			quadPoints[i].x = 2.0*(float)idxCol/aSettings->windowWidth - 1.0f + halfWidth;
			quadPoints[i].y = 1.0f - 2.0*(float)(idxRow+1)/aSettings->windowHeight - halfWidth;
			quadPoints[i].z = 0.0f;
		}
		if(i%4==2){ //bottom right of a quad
			quadPoints[i].x = 2.0*(float)(idxCol+1)/aSettings->windowWidth - 1.0f + halfWidth;
			quadPoints[i].y = 1.0f - 2.0*(float)(idxRow+1)/aSettings->windowHeight - halfHeight;
			quadPoints[i].z = 0.0f;
		}
		...
		if(idxCol == aSettings->windowWidth){ // row of quads done
			idxCol = 0;
			idxRow++;
		}
	}
...
GLInstance::~GLInstance()
{
	...
	glfwTerminate();
	std::cout << "GL Instance Destroyed" << std::endl;
}
